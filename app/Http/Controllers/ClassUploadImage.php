...
class ClassUploadImage extends Controller
{
        private function tempateCardMember($idMember)
        {
            ...
            if($idMember=='N')
            {
                // Type Member Membership Special Libur Sekolah, event baru untuk member yang berlaku di bulan juni - juni 2027
                return 'img/template-membership-special-libur-sekolah.png';
            }
            if($idMember=='AA')
            {
                // Type Member Membership Spesial Kampung Natal
        ...
        public function mergeImages($idUserClient,$typeMember,$idMember,$expiredDate)
        {
            ...
            if($typeMember=='N')
            {
                $path = $this->memberN($idUserClient,$typeMember,$idMember,$expiredDate);
            }
            // member Panarama
            if($typeMember=='AA')
            {
        ...
        private function memberN($idUserClient,$typeMember,$idMember,$expiredDate)
        {
            // call template image
            $urlTemplate = $this->tempateCardMember($typeMember);
            $image1 = Image::make(public_path($urlTemplate));
            ...
            $birthday = date('d-m-Y', strtotime($birthday));
            $image1->text($birthday, 1090, 700, function ($font) {
                $font->file(public_path('alright-sans-black.ttf'));
                $font->size(100);
                $font->color('#ffffff');
                $font->align('center');
